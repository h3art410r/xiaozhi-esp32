...
## 2026-09-14 无线麦改版：持续录音 + 左右循环快捷键 + M 发送（插件 v1.2.0）
...
- **A 机合并（9-14，commit 8c3252f）**：与 A 机本地 09-07 批次（按键音效/消震/按下触发导航/LVGL 性能）合并，本地功能已嫁接到新录音模型——音效适配为 M 发送=kCueSend 上扫、M 长按暂停=kCueStop 下行琶音、恢复=kCueStart 上行琶音（恢复前 MicMuteFor(500) 盖住音效防录进去）；MicMuteFor 消震窗保留在 MicStreamLoop 发帧处；断链 errno ESP_LOGW 与远程状态提示并存。**合并坑**：① Button 类每事件单回调槽，远程给 M 新注册的 OnPressDown（ArmMicHoldTimer）会静默覆盖 A 机的菜单按下触发，已合并为一个处理器；② 冲突块手工配平时注意 remote 侧段首的 `}` 是 MicStreamLoop 的收尾；③ ShowMicPage 注释里"M cycles/RIGHT sends"是过时注释，实际 L/R 循环、M 发送。已在 A 机编译+刷机通过
